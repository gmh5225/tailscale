...
func TestFileOps(t *testing.T) {
	...
	_, err = s.fs.OpenFile(context.Background(), pathTo(remote1, share11, "nonexistent.txt"), os.O_RDONLY, 0)
	qt.Assert(t, err, qt.IsNotNil, qt.Commentf("opening non-existent file for read should fail"))

	dir, err := s.fs.OpenFile(context.Background(), pathutil.Join(domain, remote1), os.O_RDONLY, 0)
	...
	readOnlyFile, err := s.fs.OpenFile(context.Background(), pathTo(remote1, share11, file111), os.O_RDONLY, 0)
	...
	_, err = readOnlyFile.Seek(1, io.SeekStart)
	qt.Assert(t, err, qt.IsNotNil, qt.Commentf("seeking 1 from start of read-only file should fail"))
	...
	writeOnlyFile, err := s.fs.OpenFile(context.Background(), pathTo(remote1, share11, file111), os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0)
...
func fileInfoToStatic(fi fs.FileInfo, fixupMode bool) fs.FileInfo {
	...
	return &shared.StaticFileInfo{
		...
		Sized:    fi.Size(),
		Moded:    mode,
		ModTimed: fi.ModTime().Truncate(1 * time.Second).UTC(),
		Dir:      fi.IsDir(),
	}
}
